...
import { generateReply, generateTopic } from './ai.js'
...
export async function doModeratorAction(page: Page, account: Account): Promise<boolean> {
  try {
    const aiTopic = await generateTopic('moderator', account)
    const topic = aiTopic ?? randomModeratorTopic()
...
export async function doAssistantAction(page: Page, account: Account): Promise<boolean> {
  try {
    ...
    const reply = aiReply ?? randomAssistantReply()
...
export async function doCommentatorAction(page: Page, account: Account): Promise<boolean> {
  try {
    ...
    const reply = aiReply ?? randomCommentatorReply()
...
export async function doVisitorAction(page: Page, account: Account): Promise<boolean> {
  try {
    ...
    const reply = aiReply ?? randomVisitorReply()
...
export async function doNewMemberAction(page: Page, account: Account): Promise<boolean> {
  try {
    const aiTopic = await generateTopic('newMember', account)
    const topic = aiTopic ?? randomNewMemberTopic(account.name)
    if (aiTopic) console.log(`  🧠 New-member topic generated by AI for ${account.name}`)
