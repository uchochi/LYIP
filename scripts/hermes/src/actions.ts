...
import {
  ...
  randomVisitorReply,
  randomNewMemberReply,
} from './prompts.js'
import { generateReply, generateTopic, sanitizeContent } from './ai.js'
...
export async function doModeratorAction(page: Page, account: Account): Promise<boolean> {
  try {
    const aiTopic = await generateTopic(account)
    const topic = aiTopic
      ? {
          title: sanitizeContent(aiTopic.title, account),
          content: sanitizeContent(aiTopic.content, account),
          tags: aiTopic.tags.map((t) => sanitizeContent(t, account)),
        }
      : randomModeratorTopic()
    if (aiTopic) console.log(`  🧠 Moderator topic generated by AI for ${account.name}`)
...
export async function doAssistantAction(page: Page, account: Account): Promise<boolean> {
  try {
    ...
    const aiReply = await generateReply('assistant', account)
    const reply = aiReply ? sanitizeContent(aiReply, account) : randomAssistantReply()
    if (aiReply) console.log(`  🧠 Assistant reply generated by AI for ${account.name}`)
...
export async function doCommentatorAction(page: Page, account: Account): Promise<boolean> {
  try {
    ...
    const aiReply = await generateReply('commentator', account)
    const reply = aiReply ? sanitizeContent(aiReply, account) : randomCommentatorReply()
    if (aiReply) console.log(`  🧠 Commentator reply generated by AI for ${account.name}`)
...
export async function doVisitorAction(page: Page, account: Account): Promise<boolean> {
  try {
    ...
    const aiReply = await generateReply('visitor', account)
    const reply = aiReply ? sanitizeContent(aiReply, account) : randomVisitorReply()
    if (aiReply) console.log(`  🧠 Visitor reply generated by AI for ${account.name}`)
...
export async function doNewMemberAction(page: Page, account: Account): Promise<boolean> {
  // New members cannot create topics — only admin/moderator accounts can.
  // They comment and engage in existing topics instead.
  try {
    const ids = await fetchVisibleTopics(page)
    if (ids.length === 0) {
      console.log(`  ⚠️  No topics found for new member ${account.name}`)
      return false
    }
    ...
    const aiReply = await generateReply('newMember', account)
    const reply = aiReply ? sanitizeContent(aiReply, account) : randomNewMemberReply()
    if (aiReply) console.log(`  🧠 New-member reply generated by AI for ${account.name}`)

    const textarea = page.locator('textarea')
    if ((await textarea.count()) === 0) {
      console.log(`  ⚠️  No reply form on topic for new member ${account.name}`)
      return false
    }
    ...
    if (await submitBtn.isVisible()) {
      ...
      await page.waitForTimeout(3000)
      await maybeReact(page, account)
      console.log(`  ✅ New member ${account.name} commented on topic ${topicId.slice(0, 8)}`)
      return true
    }
